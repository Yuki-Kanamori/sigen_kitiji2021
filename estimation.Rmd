...
c20_oki = data.frame(catch_kg = NA, year = 2020, method = "沖底", sum = new_catchF %>% filter(year == 2019) %>% select(catch) %>% dplyr::rename(sum = catch))
...
catchF = catch2 %>% filter(year > 1994) %>% dplyr::rename(catch = total_catch_t) #図5に使ったデータ
summary(catchF)
