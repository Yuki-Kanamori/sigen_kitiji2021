...
table_s2 = data.frame(year = rep((n_year-5):n_year+1), 資源量_tons = abind(trend %>% dplyr::filter(year > (n_year-5)) %>% select(total), total_biomass_next, along = 1), 親魚量 = abind(srr %>% filter(year2 > (n_year-5)) %>% mutate(biomass2 = biomass/1000000) %>% select(biomass2), NA, along = 1), 漁獲量_tons = abind("not_change", "not_change", "not_change", total_catch_pref, catch_this, NA, along = 1), F = abind(fishing_trend %>% filter(year > (n_year-5), data == "f") %>% select(value), f_current, NA, along = 1), 漁獲割合 = abind(fishing_trend %>% filter(year > (n_year-5), data == "rate") %>% select(value), rate_this, NA, along = 1))
...
write.csv(trawl, "tableA4-1.csv", fileEncoding = fileEncoding, row.names=F)
...
N_trend %>% filter(year == n_year)
```

#### F値（3段落目）
```{r table_fishing trend1, echo=F}
fishing_trend %>% filter(data == "f") %>% select(-data, -data2) %>% dplyr::rename("F" = value) %>% filter(year == (n_year-1))
...
fishing_trend %>% filter(data == "rate") %>% select(-data, -data2) %>% dplyr::rename("%" = value) %>% filter(year == (n_year-1))
# fishing_trend %>% filter(year < 2011, data == "f") %>% spread(key = year, value = value)
# fishing_trend %>% filter(year > 2010, data == "f") %>% spread(key = year, value = value)
# fishing_trend %>% filter(year < 2011, data == "rate") %>% spread(key = year, value = value)
# fishing_trend %>% filter(year > 2010, data == "rate") %>% spread(key = year, value = value)
