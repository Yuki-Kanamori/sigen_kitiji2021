...
abund_this = left_join(abund_this, weight %>% filter(year == (n_year-1)), by = c("age")) # get the weight of last year (2020) and join the weight data into the abundance of this year (2021)
...
table_s2 = data.frame(year = rep((n_year-5):n_year+1), 資源量_ton = abind(trend %>% dplyr::filter(year > (n_year-5)) %>% select(total), total_biomass_next, along = 1), 親魚量 = abind(srr %>% filter(year2 > (n_year-5)) %>% mutate(biomass2 = biomass/1000000) %>% select(biomass2), NA, along = 1), 漁獲量_ton = abind("not_change", "not_change", "not_change", catchF %>% filter(year == (n_year-1)) %>% select(catch), catch_this, NA, along = 1), F = abind(fishing_trend %>% filter(year > (n_year-5), data == "f") %>% select(value), f_current, NA, along = 1), 漁獲割合 = abind(fishing_trend %>% filter(year > (n_year-5), data == "rate") %>% select(value), rate_this, NA, along = 1))
...
```{r fig2, echo=F, warning=F, message=F, results=F}
age = rep(1:10)
...
```{r fig7, echo=F, warning=F, message=F, results=F}
...
write.csv(fishing_trend %>% dplyr::rename(label = data2) %>% select(-data), "tableA4-9-2.csv", fileEncoding = fileEncoding, row.names=F) 
